...
void set_event_recv(struct io_uring *ring, int sockfd, void *buf, size_t len, int flags)
{
    ...
    io_uring_prep_recv(sqe, sockfd, buf, len, flags); // 为 sqe entry 填充读事件所需相关数据
...
void set_event_send(struct io_uring *ring, int sockfd, void *buf, size_t len, int flags)
{
    ...
    io_uring_prep_send(sqe, sockfd, buf, len, flags); // 为 sqe entry 填充写事件所需相关数据
    memcpy(&sqe->user_data, &accept_info, sizeof(struct conn_info));
}
...
void set_event_accept(struct io_uring *ring, int sockfd, struct sockaddr *addr, socklen_t *addrlen, int flags)
{
    ...
    io_uring_prep_accept(sqe, sockfd, (struct sockaddr *)addr, addrlen, flags); // 为 sqe entry 填充连接事件所需相关数据
    memcpy(&sqe->user_data, &accept_info, sizeof(struct conn_info));
}
...
int main(int argc, char const *argv[])
{
    ...
    io_uring_queue_init_params(ENTRIES_LENGTH, &ring, &params); // 根据参数初始化 io_uring实例
